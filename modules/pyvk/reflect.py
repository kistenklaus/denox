...
class UpsampleParams(Enum):
    TARGET_SIZE = "target_size" # (x,y)
    MODE = "mode" # : UpsampleFilterMode
    ...
    for node in traced.graph.nodes:
        ...
        if hasattr(model, str(node.target)):
            ...
            if isinstance(target, torch.nn.Conv2d):
                ...
                weights = conv2d.weight.detach().cpu().view(-1).tolist();
                bias = conv2d.bias.detach().cpu().view(-1).tolist();
            ...
            if isinstance(target, torch.nn.MaxPool2d):
                ...
                layer.output_shape = current_shape
                layer.set_parameter(MaxPoolParams.KERNEL_SIZE, kernel_size)
                layer.set_parameter(MaxPoolParams.STRIDE, stride)
                reflected_layers.append(layer)
                continue
            if isinstance(target, torch.nn.Upsample):
                ...
                if upsample.size is not None:
                    ...
                    current_shape = [current_shape[0], current_shape[1], H_out, W_out]
                    layer.set_parameter(UpsampleParams.TARGET_SIZE, (H_out, W_out));
                    
                layer.output_shape = current_shape

                if upsample.mode == "nearest":
                    layer.set_parameter(UpsampleParams.MODE, UpsampleFilterMode.NEAREST)
                else:
                    logger.warning(f"Unrecognized filter mode: {upsample.mode}")
