...
slice(ImportState &state, memory::span<const memory::optional<Tensor>> inputs,
  ...
  memory::vector<std::int64_t> stepsI64; // keep integers as requested
  if (hasSteps) {
  ...
  // ---- normalize axes ----
  auto normalize_axes = [&](std::size_t rank) -> memory::vector<std::size_t> {
    memory::vector<std::size_t> out;
    out.reserve(nSpec);
    if (hasAxes) {
      for (auto a : axes) {
        if (a < 0)
  ...
  if (dataT.isDevice()) {
    ...
    // default steps → 1
    memory::vector<std::int64_t> stepsDev(nSpec, 1);
    if (hasSteps)
      stepsDev = stepsI64;
  ...
  const memory::vector<std::int64_t> endsI64 = read_i64_1d(endsH);
  memory::vector<std::int64_t> stepsHost(nSpec, 1);
  if (hasSteps)
    stepsHost = stepsI64;
