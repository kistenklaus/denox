...
memory::string preprocess_shader_src_pragmas(memory::string &src) {
  ...
  for (memory::string line; std::getline(ss, line);) {
  ...
  std::size_t startOfBlock = npos;
  memory::string blockId;
  for (std::size_t i = 0; i < lines.size(); ++i) {
    if (startOfBlock == npos) {
      ...
      if (std::regex_search(lines[i], m, end_block_syntax)) {
        ...
        memory::string blockSrc;
        for (std::size_t i = startOfBlock + 1; i < endOfBlock; ++i) {
          blockSrc.append(lines[i]);
          blockSrc.push_back('\n');
        }
        blocks.emplace_back(blockId, std::move(blockSrc));
        auto s = lines.begin() + static_cast<std::ptrdiff_t>(startOfBlock);
        auto e = lines.begin() + static_cast<std::ptrdiff_t>(endOfBlock) + 1;
        lines.erase(s, e);
        i -= static_cast<std::size_t>(std::distance(s, e));
        startOfBlock = npos;
  ...
  ofStream.write(outsrc.c_str(), static_cast<std::streamsize>(outsrc.size()));
