...
void SymGraph::affine_add_sym(AffineExpr &lhs, symbol s, value_type factor) {
  ...
  auto it = denox::algorithm::lower_bound(
      v.begin(), v.end(), s,
...
denox::memory::optional<symbolic::details::AffineExpr>
...
denox::memory::optional<SymGraph::AffineExpr> SymGraph::affine_div(const AffineExpr &lhs,
                                                         value_type rhs) {
  assert(rhs > 0);
  if (rhs == 1)
    return lhs;
  ...
  for (auto const &c : lhs.coef) {
    if (c.factor % rhs != 0) {
      return denox::memory::nullopt; // non-affine residual -> bail
    }
...
denox::memory::optional<SymGraph::AffineExpr>
  ...
  if (lhs.coef.size() == rhs.coef.size()) {
    ...
    if (equalSymbols) {
      ...
      for (const auto &coef : lhs.coef) {
        k = denox::algorithm::gcd(k, std::abs(coef.factor));
      }
      if (lhs.coef.empty()) {
        k = 1;
      }
      ...
      for (const auto &coef : rhs.coef) {
        h = denox::algorithm::gcd(h, std::abs(coef.factor));
      }
      AffineExpr B = rhs;
...
SymGraph::affine_mod(const AffineExpr &lhs, const value_type &rhs) {
  assert(rhs > 0);
  if (rhs == 1) {
    return 0;
  }
...
denox::memory::optional<SymGraph::AffineExpr>
SymGraph::affine_mod(const AffineExpr &lhs, const AffineExpr &rhs) {
  if (lhs.isPureConstant()) {
    ...
    if (rhs.isPureConstant()) {
      ...
      const static auto full_gcd = [](const AffineExpr &e) -> value_type {
        value_type g = 0;
        for (auto &t : e.coef)
          g = denox::algorithm::gcd(g, std::abs(t.factor));
        g = denox::algorithm::gcd(g, std::abs(e.constant));
