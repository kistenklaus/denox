...
DirectConvShader::DirectConvShader(spirv::GlslCompiler *compiler,
  ...
  if (options.optimizationLevel >= 5) {
    ...
    while (!ss.eof()) {
      ...
      if ((config.invoc_m % 8 != 0) || (config.invoc_k % 8 != 0) ||
          (config.invoc_k % 8 != 0)) {
        continue;
      }
      ...
      static constexpr size_t PIPELINE_DEPTH = 2;
      const uint32_t sh_a_size =
          PIPELINE_DEPTH * (config.sg_m * config.wg_m * config.invoc_m *
                            config.sg_k * config.invoc_k * 2);
      const uint32_t sh_b_size =
          PIPELINE_DEPTH * (config.invoc_n * config.sg_n * config.wg_n *
                            config.sg_k * config.invoc_k * 2);
      const uint32_t sh_size = sh_a_size + sh_b_size;
      ...
      const uint32_t k8_tile = config.invoc_k / 8 * config.sg_k;
      if (k8_tile % config.wg_n != 0) {
        continue;
      }
      if (k8_tile < config.wg_n) {
        continue;
      }
  ...
  if (options.features.enableConvReluFusion) { // possibly more patterns.
    ...
    relu->matchValue([](const ComputeOp &op) {
      ...
      if (op.activation().func.kind() != ActivationFunctionKind::ReLU) {
        return false;
      }
      return true;
    });
...
direct_conv_compile(spirv::GlslCompiler *compiler, const io::Path &srcPath,
                    ...
                    memory::uvec2 stride, bool bias, bool subgroupControl,
...
void DirectConvShader::implement(
  ...
  if (activationFunction) {
    ...
    case ActivationFunctionKind::Swish:
      dispatch.setOperation(fmt::format(
          "swich(conv2d(x,kernel_size=({},{}),bias={},stride=({},"
