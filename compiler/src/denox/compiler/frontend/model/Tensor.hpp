...
public:
  ...
  void setType(TensorDataType type) {
    m_controlBlock->hypergraph.get(m_nodeId).type = type;
  }
  ...
  void setStorage(TensorStorage storage) {
    m_controlBlock->hypergraph.get(m_nodeId).storage = storage;
  }
  ...
  void setFormat(TensorFormat format) {
    m_controlBlock->hypergraph.get(m_nodeId).format = format;
  }
