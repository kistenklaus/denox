...
concat(ImportState &state, memory::span<const memory::optional<Tensor>> inputs,
  ...
  // Device path: exactly 2 device tensors, channel axis
  {
  ...
  // ---------- Host path (variadic) ----------
  memory::vector<const HostTensor *> hs;
  ...
  memory::vector<std::uint64_t> outU64 = d0;
  ...
  auto prod = [](const memory::vector<std::uint64_t> &v, std::size_t from,
                 std::size_t to) {
    std::size_t p = 1;
    for (std::size_t i = from; i < to; ++i)
      p *= static_cast<std::size_t>(v[i]);
    return p;
  };
  const std::size_t outerCount = prod(outU64, 0, A);
  const std::size_t innerElems = prod(outU64, A + 1, R);
  std::size_t totalElems =
      outerCount * innerElems * static_cast<std::size_t>(outU64[A]);
  if (totalElems == 0) {
  ...
  memory::vector<HostTensor> contig;
  contig.reserve(hs.size());
  for (auto *h : hs)
    contig.push_back(h->contiguous());

  // Per-input extent along axis A
  memory::vector<std::size_t> lenA(hs.size());
  for (std::size_t i = 0; i < hs.size(); ++i)
    lenA[i] = static_cast<std::size_t>(contig[i].shape().toU64()[A]);
  ...
  if (!promoteToSym) {
    ...
    for (std::size_t o = 0; o < outerCount; ++o) {
      std::size_t doff = o * innerBytes * static_cast<std::size_t>(outU64[A]);
      for (std::size_t i = 0; i < contig.size(); ++i) {
