...
memory::vector<Tensor> average_pool(
  ...
  if (inputs.size() != 1 || !inputs[0].has_value())
    throw std::runtime_error(fmt::format(
        "AveragePool \"{}\" expects exactly 1 input.", nodeName));
  if (outputCount != 1)
    throw std::runtime_error(fmt::format(
        "AveragePool \"{}\": expected exactly 1 output.", nodeName));

  const Tensor &X = *inputs[0];
  if (!X.isDevice())
    throw std::runtime_error(
        "AveragePool: only DeviceTensor is supported.");

  const DeviceTensor Xdev = X.device();
  const std::size_t r = Xdev.rank();
  if (r != 3 && r != 4)
    throw std::runtime_error(fmt::format(
        "AveragePool \"{}\": input must be CHW or NCHW.", nodeName));
  ...
  if (auto it = attributes.find("auto_pad"); it != attributes.end()) {
    if (!it->second.isString())
      throw std::runtime_error(fmt::format(
          "AveragePool \"{}\": auto_pad must be string, got {}.",
          nodeName, it->second.kindName()));
    if (it->second.s() != "NOTSET")
      throw std::runtime_error(
          "AveragePool: only auto_pad=\"NOTSET\" is supported.");
  }
  ...
  if (auto it = attributes.find("ceil_mode"); it != attributes.end()) {
    if (!it->second.isInt())
      throw std::runtime_error(fmt::format(
          "AveragePool \"{}\": ceil_mode must be int, got {}.", nodeName,
          it->second.kindName()));
    if (it->second.i() != 0)
      throw std::runtime_error(
  ...
  memory::uvec2 kernel(0, 0);
  {
    ...
    if (it == attributes.end())
      throw std::runtime_error("AveragePool: kernel_shape is required.");
    const Attribute &a = it->second;
    if (!a.isInts())
      throw std::runtime_error(fmt::format(
          "AveragePool \"{}\": kernel_shape must be ints, got {}.",
          nodeName, a.kindName()));
    const auto &v = a.ints();
    if (v.size() != 2)
      throw std::runtime_error(
          fmt::format("AveragePool \"{}\": kernel_shape must have size "
                      "2 (H,W), got {}.",
                      nodeName, v.size()));
    if (v[0] < 1 || v[1] < 1)
      throw std::runtime_error("AveragePool: kernel dims must be >= 1.");
  ...
  if (auto it = attributes.find("strides"); it != attributes.end()) {
    ...
    if (!a.isInts())
      throw std::runtime_error(fmt::format(
          "AveragePool \"{}\": strides must be ints, got {}.", nodeName,
          AttributeKind_name(a.kind())));
    const auto &v = a.ints();
    if (v.size() != 2)
      throw std::runtime_error(fmt::format(
          "AveragePool \"{}\": strides must have size 2 (H,W), got {}.",
          nodeName, v.size()));
    if (v[0] < 1 || v[1] < 1)
      throw std::runtime_error("AveragePool: strides must be >= 1.");
  ...
  if (auto it = attributes.find("pads"); it != attributes.end()) {
    ...
    if (!a.isInts())
      throw std::runtime_error(
          fmt::format("AveragePool \"{}\": pads must be ints, got {}.",
                      nodeName, a.kindName()));
    const auto &v = a.ints();
    if (v.size() == 2) {
      if (v[0] < 0 || v[1] < 0)
        throw std::runtime_error("AveragePool: pads must be >= 0.");
      padding.y = static_cast<unsigned>(v[0]); // H
      padding.x = static_cast<unsigned>(v[1]); // W
    } else if (v.size() == 4) {
      if (v[0] != v[2] || v[1] != v[3])
        throw std::runtime_error(
            "AveragePool: asymmetric pads not supported "
      ...
      throw std::runtime_error(fmt::format(
          "AveragePool \"{}\": pads must have size 2 or 4, got {}.",
          nodeName, v.size()));
    }
  }
