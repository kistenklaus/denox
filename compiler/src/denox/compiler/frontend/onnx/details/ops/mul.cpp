...
mul(ImportState &state, memory::span<const memory::optional<Tensor>> inputs,
  ...
  if (inputs.size() != 2 || !inputs[0].has_value() || !inputs[1].has_value())
    throw std::runtime_error(
        fmt::format("vkcnn: Mul \"{}\" expects 2 inputs.", nodeName));
  if (outputCount != 1)
    throw std::runtime_error(
        fmt::format("vkcnn: Mul \"{}\" must have exactly 1 output.", nodeName));
  ...
  if (aT.isDevice() || bT.isDevice())
    throw std::runtime_error(fmt::format(
        "vkcnn: Mul \"{}\": runtime tensors not supported.", nodeName));
  ...
  if (!a0.isConstant() || !b0.isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Mul \"{}\": dynamic host tensors unsupported.", nodeName));
  ...
  if (adt == Dtype::String || bdt == Dtype::String || adt == Dtype::Bool ||
      bdt == Dtype::Bool)
    throw std::runtime_error(fmt::format(
        "vkcnn: Mul \"{}\": unsupported dtype (string/bool).", nodeName));
  ...
  if (!viewA.isConstant() || !viewB.isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Mul \"{}\": non-constant broadcast view.", nodeName));
  ...
  if (adt == Dtype::Sym || bdt == Dtype::Sym) {
    if (adt.isFloat() || bdt.isFloat())
      throw std::runtime_error(fmt::format(
          "vkcnn: Mul \"{}\": symbolic with floats not supported.", nodeName));
    ...
    while (true) {
      ...
      Sym r =
          state.symGraph->mul(xs, ys); // no positivity assumptions
      *po++ = r;
      if (!inc())
        break;
    }
  ...
  if (!(adt.isInteger() && bdt.isInteger()))
    throw std::runtime_error(fmt::format(
        "vkcnn: Mul \"{}\": unsupported dtype combination.", nodeName));
