...
cast([[maybe_unused]] ImportState &state,
  ...
  if (inputs.size() != 1 || !inputs[0].has_value())
    throw std::runtime_error(
        fmt::format("Cast \"{}\" expects exactly 1 input.", nodeName));
  if (outputCount != 1)
    throw std::runtime_error(fmt::format(
        "Cast \"{}\" must have exactly 1 output.", nodeName));
  ...
  if (inT.isDevice())
    throw std::runtime_error(fmt::format(
        "Cast \"{}\": runtime tensors not supported.", nodeName));
  ...
  if (it == attributes.end() || !it->second.isInt())
    throw std::runtime_error(fmt::format(
        "Cast \"{}\": missing required int attribute 'to'.", nodeName));
  ...
  if (!to_dtype_opt.has_value())
    throw std::runtime_error(fmt::format(
        "Cast \"{}\": unsupported target dtype {}.", nodeName, to_onxx));
  ...
  if (src_dtype == Dtype::Sym) {
    ...
    if (to_dtype.isFloat() || to_dtype == Dtype::String) {
      throw std::runtime_error(fmt::format(
          "Cast \"{}\": cannot cast Symbolic to float or string.",
          nodeName));
    }
    // Any other target kind is unsupported
    throw std::runtime_error(fmt::format(
        "Cast \"{}\": unsupported cast from Sym.", nodeName));
  }
  ...
  if (src_dtype == Dtype::String) {
    throw std::runtime_error(fmt::format(
        "Cast \"{}\": string casts are not supported.", nodeName));
  }
  ...
  // implemented)
  throw std::runtime_error(fmt::format(
      "Cast \"{}\": non-noop cast from {} to {} not supported yet.",
