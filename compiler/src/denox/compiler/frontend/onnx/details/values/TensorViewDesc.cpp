...
TensorViewDesc TensorViewDesc::Identity(const TensorShape &shape) {
  ...
  memory::vector<Symbolic> strides(r);
...
memory::span<const Symbolic> TensorViewDesc::strides() const {
  return m_strides;
}
const Symbolic &TensorViewDesc::offset() const { return m_offset; }
...
Symbolic
TensorViewDesc::indexOfSym(memory::span<const Symbolic> idx) const {
  assert(idx.size() == m_strides.size());
...
TensorViewDesc::permute(memory::span<const std::int64_t> perm) const {
  ...
  memory::vector<Symbolic> s(perm.size());
...
TensorViewDesc TensorViewDesc::normalizeNegativeStrides(
    std::span<const Symbolic> shape) const {
...
TensorViewDesc TensorViewDesc::unsqueeze(size_t axis) const {
  ...
  s.insert(s.begin() +
               static_cast<memory::vector<Symbolic>::difference_type>(
                   axis),
           stride_here);
  return TensorViewDesc{std::move(s), m_offset};
}
...
TensorViewDesc::TensorViewDesc(std::vector<Symbolic> strides,
                               Symbolic offset)
