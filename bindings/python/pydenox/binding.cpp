...
static denox::FeatureState parse_feature_state(const py::object &o) {
  if (o.is_none())
    return denox::FeatureState::Enable; // default like CLI
  if (py::isinstance<py::bool_>(o))
    return o.cast<bool>() ? denox::FeatureState::Enable
                          : denox::FeatureState::Disable;
  auto s = o.cast<std::string>();
  for (auto &c : s)
    c = char(::tolower(unsigned(c)));
  if (s == "require")
    return denox::FeatureState::Require;
  if (s == "enable")
    return denox::FeatureState::Enable;
  if (s == "disable")
    return denox::FeatureState::Disable;
  throw std::invalid_argument(
      "feature state must be 'require'|'enable'|'disable' or bool");
}
...
static denox::Extent parse_extent_obj(const py::object &o, NameArena &arena) {
  ...
  if (pos == std::string::npos) {
    ...
    if (!rhs.empty())
      e.value = static_cast<unsigned int>(std::stoul(rhs));
    return e;
  }
}

// Accept: None, "H:W:C", or a 3-seq like (H, W, C). Elements per
// parse_extent_obj.
static denox::Shape parse_shape_obj(const py::object &o, NameArena &arena) {
  ...
  if (py::isinstance<py::sequence>(o) && !py::isinstance<py::str>(o)) {
    ...
    if (py::len(seq) != 3)
      throw std::invalid_argument("shape sequence must have length 3");
...
static CompileResultBytes do_compile_to_bytes(
    const std::string &model, std::optional<std::string> device,
    const std::string &target_env, py::object input_shape,
    py::object output_shape, const std::string &input_type,
    const std::string &output_type, const std::string &input_layout,
    const std::string &output_layout, const std::string &input_storage,
    const std::string &output_storage, py::object coopmat, py::object fusion,
    py::object memory_concat, bool spirv_debug_info,
    bool spirv_non_semantic_debug_info, bool spirv_optimize,
    bool spirv_skip_compilation, bool verbose, bool quiet, bool summarize,
    py::object cwd) {
...
static Program py_compile_program(
  ...
  auto r = do_compile_to_bytes(
      model, device, target_env, input_shape, output_shape, input_type,
      output_type, input_layout, output_layout, input_storage, output_storage,
      coopmat, fusion, memory_concat, spirv_debug_info,
      spirv_non_semantic_debug_info, spirv_optimize, spirv_skip_compilation,
...
PYBIND11_MODULE(_denox, m) {
  ...
  auto add_sig = [&](auto name, auto fn) {
    m.def(name, fn, py::arg("model"), py::kw_only(),
          ...
          py::arg("coopmat") =
              py::none(), // 'require'|'enable'|'disable' or bool
          ...
          py::arg("spirv_skip_compilation") = false, py::arg("verbose") = false,
          py::arg("quiet") = false, py::arg("summarize") = false,
          py::arg("cwd") = py::none());
  };
