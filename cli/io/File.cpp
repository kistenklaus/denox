...
File::size_type File::read(std::span<std::byte> dst) {
  std::error_code ec;
...
void File::read_exact(std::span<std::byte> dst) {
...
File::size_type File::write(std::span<const std::byte> src) {
  std::error_code ec;
...
void File::write_exact(std::span<const std::byte> src) {
...
File::size_type File::read_ec(std::span<std::byte> dst,
...
File::size_type File::write_ec(std::span<const std::byte> src,
                               std::error_code &ec) noexcept {
  if (!m_file) {
    ec = std::make_error_code(std::errc::bad_file_descriptor);
    return 0;
