...
public:
  ...
  Path() = default;
  explicit Path(std::string_view utf8) : m_native(from_utf8(utf8)) {}
  ...
  [[nodiscard]] Path with_extension(std::string_view new_ext) const;
  [[nodiscard]] Path with_filename(std::string_view new_name) const;
  friend Path operator/(const Path &a, const Path &b);
  friend Path operator/(const Path &a, std::string_view b_utf8);
  Path &operator/=(const Path &rhs);
  Path &operator/=(std::string_view rhs_utf8);
  static Path join(std::initializer_list<std::string_view> parts);
...
private:
  ...
  static std::filesystem::path from_utf8(std::string_view s);
...
struct fmt::formatter<Path> : fmt::formatter<std::string_view> {
  ...
  auto format(const Path& p, FormatContext& ctx) const {
    // Use UTF-8 representation for CLI output
