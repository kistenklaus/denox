...
#include "parser/actions/bench.hpp"
#include "parser/actions/compile.hpp"
#include "parser/actions/infer.hpp"
#include "parser/actions/populate.hpp"
#include "parser/actions/help.hpp"
...
class Action {
public:
  Action(CompileAction a) noexcept
    : m_value(std::move(a)) {}

  Action(InferAction a) noexcept
    : m_value(std::move(a)) {}

  Action(BenchAction a) noexcept
    : m_value(std::move(a)) {}

  Action(PopulateAction a) noexcept
    : m_value(std::move(a)) {}

  Action(HelpAction a) noexcept
    : m_value(std::move(a)) {}

  static Action version() noexcept {
    return Action{VersionTag{}};
  }
...
private:
  ...
  explicit Action(VersionTag) noexcept
    : m_value(VersionTag{}) {}
