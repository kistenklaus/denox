...
import_op_Max(ImportState &state, std::span<const std::optional<Tensor>> inputs,
  ...
  if (inputs.empty())
    throw std::runtime_error(fmt::format(
        "vkcnn: Max \"{}\" expects at least 1 input.", node.name()));
  if (outputCount != 1)
    throw std::runtime_error(fmt::format(
        "vkcnn: Max \"{}\" must have exactly 1 output.", node.name()));
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    if (!inputs[i].has_value())
      throw std::runtime_error(fmt::format(
          "vkcnn: Max \"{}\": input {} is missing.", node.name(), i));
    ...
    if (!h.view().isConstant())
      throw std::runtime_error(
          fmt::format("vkcnn: Max \"{}\": input {} must have constant view.",
                      node.name(), i));
    Xs.push_back(&h);
  }
  ...
  if (!all_same_dtype) {
    throw std::runtime_error(
        fmt::format("vkcnn: Max \"{}\": all inputs must have the same dtype.",
                    node.name()));
  }
