...
static std::vector<Tensor> import_op_Clip(
  ...
  if (inputs.size() < 1 || inputs.size() > 3)
    throw std::runtime_error(
        fmt::format("vkcnn: Clip \"{}\" expects 1..3 inputs (X, [min], [max]).",
                    node.name()));
  if (outputCount != 1)
    throw std::runtime_error(fmt::format(
        "vkcnn: Clip \"{}\" must have exactly 1 output.", node.name()));

  // ---- X ----
  if (!inputs[0].has_value())
    throw std::runtime_error(
        fmt::format("vkcnn: Clip \"{}\": input X is required.", node.name()));
  const Tensor &X = *inputs[0];
  if (!X.isHost())
    throw std::runtime_error(fmt::format(
        "vkcnn: Clip \"{}\": only HostTensor is supported.", node.name()));
  const HostTensor &Xin = X.host();

  // Static shape + constant view (we rely on constIndexOf)
  if (!Xin.isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Clip \"{}\": X must have constant shape.", node.name()));
  if (!Xin.view().isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Clip \"{}\": X must have constant view.", node.name()));
