...
static std::vector<Tensor> import_op_Resize(
  ...
  if (outputCount != 1)
    throw std::runtime_error(fmt::format(
        "vkcnn: Resize \"{}\" must have exactly 1 output.", node.name()));
  ...
  if (auto it = attributes.find("antialias"); it != attributes.end()) {
    if (!it->second.isInt() || it->second.i() != 0)
      throw std::runtime_error(fmt::format(
          "vkcnn: Resize \"{}\": antialias not supported.", node.name()));
  }
  ...
  if (inputs.size() >= 2 && inputs[1].has_value()) {
    ...
    if (roi.sizeElemsIfStatic() != 0)
      throw std::runtime_error(
          fmt::format("vkcnn: Resize \"{}\": non-empty roi is not supported.",
                      node.name()));
  }
  ...
  if (!(inputs.size() >= 3 && inputs[2].has_value()))
    throw std::runtime_error(fmt::format(
        "vkcnn: Resize \"{}\": missing scales input.", node.name()));

  const Tensor &scalesT = *inputs[2];
  if (scalesT.isDevice())
    throw std::runtime_error(fmt::format(
        "vkcnn: Resize \"{}\": scales must be a host tensor.", node.name()));
  const HostTensor &scalesH = scalesT.host();

  if (!scalesH.isConstant() || scalesH.type() != Dtype::Float32)
    throw std::runtime_error(fmt::format(
        "vkcnn: Resize \"{}\": scales must be constant Float32.", node.name()));
  if (!scalesH.isContiguous())
    throw std::runtime_error(fmt::format(
        "vkcnn: Resize \"{}\": scales must be contiguous.", node.name()));
  ...
  const auto scales = scalesH.span<float>();
  if (scales.size() != rank)
    throw std::runtime_error(fmt::format(
        "vkcnn: Resize \"{}\": scales length ({}) must equal input rank ({}).",
        node.name(), scales.size(), rank));
  ...
  if (sHf <= 0.0f || sWf <= 0.0f)
    throw std::runtime_error(fmt::format(
        "vkcnn: Resize \"{}\": spatial scales must be positive.", node.name()));
  if (!near_eq(sHf, sWf))
    throw std::runtime_error(fmt::format(
  ...
  if (!near_eq(sHf, sRound))
    throw std::runtime_error(
        fmt::format("vkcnn: Resize \"{}\": spatial scale must be an integer.",
                    node.name()));
  const unsigned int s = static_cast<unsigned int>(sRound);
  if (s < 1)
    throw std::runtime_error(fmt::format(
        "vkcnn: Resize \"{}\": integer scale must be >= 1.", node.name()));
