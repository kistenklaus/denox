...
import_op_Div(ImportState &state, std::span<const std::optional<Tensor>> inputs,
  ...
  if (inputs.size() != 2 || !inputs[0].has_value() || !inputs[1].has_value())
    throw std::runtime_error(
        fmt::format("vkcnn: Div \"{}\" expects 2 inputs.", node.name()));
  if (outputCount != 1)
    throw std::runtime_error(fmt::format(
        "vkcnn: Div \"{}\" must have exactly 1 output.", node.name()));
  ...
  if (aT.isDevice() || bT.isDevice())
    throw std::runtime_error(fmt::format(
        "vkcnn: Div \"{}\": runtime tensors not supported.", node.name()));
  ...
  if (!a0.isConstant() || !b0.isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Div \"{}\": dynamic host tensors unsupported.", node.name()));
  ...
  // Disallow strings/bools outright (and we also skip Float16 here)
  ...
  if (!viewA.isConstant() || !viewB.isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Div \"{}\": non-constant broadcast view.", node.name()));
  ...
  // Load helpers (reused many times)
  ...
  // -------- Symbolic path (integer domain only) --------
  if (adt == Dtype::Sym || bdt == Dtype::Sym) {
    if (is_float32_or_64(adt) || is_float32_or_64(bdt))
      throw std::runtime_error(
          fmt::format("vkcnn: Div \"{}\": symbolic with floats not supported.",
                      node.name()));
    ...
    while (true) {
      ...
      if (ys.isConstant()) {
        ...
        if (den == 0)
          throw std::runtime_error(
              fmt::format("vkcnn: Div \"{}\": division by zero.", node.name()));
        if (den < 0)
          throw std::runtime_error(
      ...
      Sym r = state.symGraph->div(xs, ys); // create / normalize via sym engine
      *po++ = r;
      if (!inc())
        break;
    }
  ...
  if (!(is_integer(adt) && is_integer(bdt)))
    throw std::runtime_error(fmt::format(
        "vkcnn: Div \"{}\": unsupported dtype combination.", node.name()));
