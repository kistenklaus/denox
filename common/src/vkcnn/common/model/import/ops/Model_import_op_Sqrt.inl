...
static std::vector<Tensor> import_op_Sqrt(
  ...
  if (inputs.size() != 1 || !inputs[0].has_value())
    throw std::runtime_error(fmt::format(
        "vkcnn: Sqrt \"{}\" expects exactly 1 input.", node.name()));
  if (outputCount != 1)
    throw std::runtime_error(fmt::format(
        "vkcnn: Sqrt \"{}\" must have exactly 1 output.", node.name()));

  const Tensor &X = *inputs[0];
  if (!X.isHost())
    throw std::runtime_error(fmt::format(
        "vkcnn: Sqrt \"{}\": only HostTensor is supported.", node.name()));
  ...
  if (!Xin.isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Sqrt \"{}\": input must have constant shape.", node.name()));
  if (!Xin.view().isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Sqrt \"{}\": input must have constant view.", node.name()));
