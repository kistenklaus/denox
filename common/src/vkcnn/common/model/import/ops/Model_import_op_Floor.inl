...
static std::vector<Tensor> import_op_Floor(
  ...
  if (inputs.size() != 1)
    throw std::runtime_error(
        fmt::format("vkcnn: Floor \"{}\" expects 1 input.", node.name()));
  if (!inputs[0].has_value())
    throw std::runtime_error(
        fmt::format("vkcnn: Floor \"{}\": input is required.", node.name()));
  if (outputCount != 1)
    throw std::runtime_error(fmt::format(
        "vkcnn: Floor \"{}\" must have exactly 1 output.", node.name()));
  ...
  if (!xT.isHost())
    throw std::runtime_error(fmt::format(
        "vkcnn: Floor \"{}\": DeviceTensor is not supported.", node.name()));
  ...
  if (!Xc.shape().isConstant())
    throw std::runtime_error(
        fmt::format("vkcnn: Floor \"{}\": dynamic host shapes are unsupported.",
                    node.name()));
