...
import_op_Neg(ImportState &state, std::span<const std::optional<Tensor>> inputs,
  ...
  if (inputs.size() != 1 || !inputs[0].has_value())
    throw std::runtime_error(
        fmt::format("vkcnn: Neg \"{}\" expects exactly 1 input.", node.name()));
  if (outputCount != 1)
    throw std::runtime_error(fmt::format(
        "vkcnn: Neg \"{}\" must have exactly 1 output.", node.name()));

  const Tensor &X = *inputs[0];
  if (!X.isHost())
    throw std::runtime_error(fmt::format(
        "vkcnn: Neg \"{}\": only HostTensor is supported.", node.name()));
  ...
  if (!Xin.isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Neg \"{}\": input must have constant shape.", node.name()));
  if (!Xin.view().isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Neg \"{}\": input must have constant view.", node.name()));
  ...
  if (isI32) {
    ...
    for (std::size_t i = 0; i < N; ++i) {
      ...
      if (v == std::numeric_limits<std::int32_t>::min())
        throw std::runtime_error(fmt::format(
            "vkcnn: Neg \"{}\": neg(INT32_MIN) overflows.", node.name()));
      dst[i] = -v;
      if (!next_indexer(idx))
        break;
    }
  ...
  if (isI64) {
    ...
    for (std::size_t i = 0; i < N; ++i) {
      ...
      if (v == std::numeric_limits<std::int64_t>::min())
        throw std::runtime_error(fmt::format(
            "vkcnn: Neg \"{}\": neg(INT64_MIN) overflows.", node.name()));
      dst[i] = -v;
      if (!next_indexer(idx))
        break;
    }
