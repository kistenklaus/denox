...
static std::vector<Tensor> import_op_Pow(
  ...
  if (inputs.size() != 2 || !inputs[0].has_value() || !inputs[1].has_value())
    throw std::runtime_error(fmt::format(
        "vkcnn: Pow \"{}\" expects exactly 2 inputs.", node.name()));
  if (outputCount != 1)
    throw std::runtime_error(fmt::format(
        "vkcnn: Pow \"{}\" must have exactly 1 output.", node.name()));
  ...
  if (!Ah.isConstant() || !Bh.isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Pow \"{}\": inputs must have constant shapes.", node.name()));
  if (!Ah.view().isConstant() || !Bh.view().isConstant())
    throw std::runtime_error(fmt::format(
        "vkcnn: Pow \"{}\": inputs must have constant views.", node.name()));
  ...
  if ((!aF32 && !aF64) || (!bF32 && !bF64)) {
    throw std::runtime_error(
        fmt::format("vkcnn: Pow \"{}\": unsupported dtypes (A={}, B={}). "
                    ...
                    node.name(), dtype_to_string(adt), dtype_to_string(bdt)));
  }
  ...
  // Index buffers
  std::vector<std::uint64_t> oIdx(rO, 0);
  std::vector<std::uint64_t> aIdx(rA, 0);
  std::vector<std::uint64_t> bIdx(rB, 0);
  ...
  if (outIsF64) {
    ...
    const auto AsrcF32 = aF32 ? Ah.storage()->f32() : std::span<const float>{};
    const auto AsrcF64 = aF64 ? Ah.storage()->f64() : std::span<const double>{};
    const auto BsrcF32 = bF32 ? Bh.storage()->f32() : std::span<const float>{};
    const auto BsrcF64 = bF64 ? Bh.storage()->f64() : std::span<const double>{};
